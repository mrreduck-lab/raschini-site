  ...
  return (
    <main>
      ...
      <section id="top" className="hero heroVideo">
        ...
        <div className="heroContent"><p className="heroKicker">Весна — лето 2026</p><h1><span>Сила</span><span>спокойствия</span></h1><p className="heroLead">Неаполитанская лёгкость.<br/>Современный ритм.</p><a className="cta" href="https://raschini.com/new/"><span>Открыть коллекцию</span><ArrowIcon/></a></div>
      ...
      <section className="journalGrid" data-reveal><article className="journalIntro lightPanel"><p>Raschini Journal</p><h2>Идеи.<br/>Люди.<br/>Стиль.</h2><i /><span>Путешествия, культура и неаполитанское ремесло — в материалах Raschini и ведущих редакций.</span><a href="https://ymag.media/articles/muzhchina-v-otpuske" target="_blank" rel="noreferrer">Читать материалы <ArrowIcon/></a></article>{articles.map(({ kicker, title, note, cls, href }, index) => <a className={`story story-${cls}`} key={title} href={href} target="_blank" rel="noreferrer" style={{ '--delay': `${index * 90}ms` } as React.CSSProperties}><div className="storyMedia"/><div className="shade"/><p>{kicker}<small>{note}</small></p><h3>{title}</h3><span className="storyArrow"><ArrowIcon/></span></a>)}</section>
