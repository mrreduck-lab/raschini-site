...
export default function PwaStartPage() {
  ...
  if (state === 'connecting') {
    return <main className="screen"><section className="card centered"><p className="eyebrow">Raschini Club</p><h1>Открываем ваш Raschini</h1><div className="loader" /></section><Styles /></main>;
  }

  if (state === 'error') {
    return <main className="screen"><section className="card"><p className="eyebrow">Raschini Club</p><h1>Ссылка устарела</h1><p>Одноразовая ссылка действует пять минут. Вернитесь на сайт и начните добавление ещё раз.</p><a className="primary" href="https://raschini.com/?raschini_pwa_connect=1">Повторить</a></section><Styles /></main>;
  }
...
function Styles() {
  return <style jsx>{`
    .screen{min-height:100svh;background:#eee8df;color:#17130f;padding:max(42px,env(safe-area-inset-top)) 20px max(36px,env(safe-area-inset-bottom));display:grid;place-items:center}.card{width:min(680px,100%)}.centered{text-align:center}.eyebrow{text-transform:uppercase;letter-spacing:.28em;font-size:10px;color:#9b7844;margin:0 0 28px}.logo{width:118px;height:auto;mix-blend-mode:multiply;margin-bottom:26px}h1{font-family:var(--font-display),serif;font-size:clamp(48px,9vw,86px);line-height:.9;font-weight:400;letter-spacing:-.035em;margin:0 0 28px}.lead,.card>p:not(.eyebrow){font-size:16px;line-height:1.65;color:rgba(23,19,15,.68);max-width:590px}.steps{margin:42px 0 34px;border-top:1px solid rgba(23,19,15,.2)}.steps article{display:grid;grid-template-columns:36px 1fr;gap:16px;padding:22px 0;border-bottom:1px solid rgba(23,19,15,.2)}.steps span{font-size:11px;color:#9b7844}.steps strong{display:block;font-family:var(--font-display),serif;font-size:27px;font-weight:400}.steps p{margin:5px 0 0;font-size:13px;line-height:1.5;color:rgba(23,19,15,.58)}.primary,.secondary{display:flex;align-items:center;justify-content:center;min-height:54px;text-decoration:none;text-transform:uppercase;letter-spacing:.16em;font-size:10px}.primary{background:#17130f;color:#fff;margin-top:30px}.secondary{color:#17130f;border:1px solid rgba(23,19,15,.35)}.note{margin:0 0 22px!important;font-size:13px!important}.loader{width:32px;height:32px;border:1px solid rgba(23,19,15,.22);border-top-color:#17130f;border-radius:50%;margin:36px auto;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
  `}</style>;
}
